...
function processRowsByLineBreaks(lines) {
    ...
    if (lineBreakPositions.length === 0) {
      ...
      lineBreakPositions.forEach(breakIndex => {
        ...
        for (let i = startIndex; i <= breakIndex; i++) {
          ...
          if (i < breakIndex && (symbol.detectedBreak === 'SPACE' || symbol.detectedBreak === 'EOL_SURE_SPACE')) {
            columnText += ' ';
          }
        }
  ...
  console.log(`基于LINE_BREAK处理后得到${tableRows.length}行，最大列数：${Math.max(...tableRows.map(row => row.length) || [0])}`);
  return tableRows;
}
...
function finalizeTableData(tableRows) {
  ...
  const isDistinctFromOtherRows = normalizedRows.length > 1 && 
    normalizedRows.slice(1).every(row => !row.every((cell, i) => 
...
function processTableByLineBreaks(symbols) {
  ...
  console.log('各行列数分布情况:', Object.entries(columnCounts).map(([cols, rows]) => `${cols}列: ${rows}行`).join(', '));
