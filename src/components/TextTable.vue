  ...
  lines.forEach(line => {
    ...
    sortedLineSymbols.forEach(symbol => {
      ...
      for (let i = 0; i < columnSeparators.length; i++) {
        if (symbolCenter < columnSeparators[i]) {
          columnIndex = i;
          break;
        }
      }
...
function finalizeTableData(tableRows) {
  ...
  const isDistinctFromOtherRows = tableRows.length > 1 && 
    tableRows.slice(1).every(row => !row.every((cell, i) => 
      cell.length > 0 && firstRow[i].length > 0 && 
...
function processMultiColumnTable(symbols, estimatedColumns) {
  ...
  const isFirstRowHeader = firstRow.some(cell => 
    cell.length > 0 && !/\d+/.test(cell) && cell.length < 20
  );
  ...
  for (let r = 0; r < rows; r++) {
    ...
    if (estimatedColumns === 2) {
      ...
      separateRows.forEach(separateRow => {
        ...
        // 分到左右两列
        separateRow.sort((a, b) => a.x - b.x).forEach(symbol => {
