    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
      <div class="bg-white rounded-lg shadow-md p-6 border-t-4 border-blue-500">
        ...
        <div class="flex items-center">
          <div class="w-3 h-3 rounded-full mr-2" :class="serverStatus.online ? 'bg-green-500' : 'bg-red-500'"></div>
      ...
      <div class="bg-white rounded-lg shadow-md p-6 border-t-4 border-green-500">
        ...
        <div class="flex items-center">
          <div class="w-3 h-3 rounded-full mr-2" :class="dbStatus.connected ? 'bg-green-500' : 'bg-red-500'"></div>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">识别语言</th>
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">创建时间</th>
              </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="record in ocrRecords" :key="record.id">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ record.userId }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ record.filename }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ record.language }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ formatDate(record.createdAt) }}</td>
              </tr>
              <tr v-if="ocrRecords.length === 0">
                <td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500">无OCR记录数据</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="mb-4 flex items-center justify-between">
          <div class="flex space-x-2">
            <button 
              ...
              :class="currentLogFilter === filter.type ? filter.activeClass : 'bg-gray-200 text-gray-700'"
            >
              {{ filter.label }}
            </button>
          </div>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="(log, index) in filteredLogs" :key="index" :class="getLogRowClass(log)">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ log.responseTime }}ms</td>
              </tr>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
      ...
      <div v-if="queryResult" class="mt-6">
        ...
        <div class="text-sm text-gray-500 mb-2">
          执行时间: {{ queryResult.executionTime }}ms | 返回行数: {{ queryResult.rows ? queryResult.rows.length : 0 }}
        </div>
        <div class="overflow-x-auto">
          <table v-if="queryResult.rows && queryResult.rows.length > 0" class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                <th 
