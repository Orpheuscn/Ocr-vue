...
// 确保正确导入你的 API 服务文件路径
import { performOcrRequest, getLanguageName, getAllLanguages } from '@/services/visionApi';
// 导入PDF适配器
...
import { processSimple, checkApiStatus } from '@/services/apiClient';
...
export const useOcrStore = defineStore('ocr', () => {
    ...
    try {
      ...
      if (useServerApiKey.value && serverApiAvailable.value) {
        ...
        try {
          ...
          if (!simpleResult || !simpleResult.text) {
            throw new Error('服务器返回的OCR结果无效');
          }
  ...
  return {
    // State
    apiKey, showApiSettings, currentFiles, filePreviewUrl, isPdfFile, pdfDataArray, currentPage, totalPages, selectedLanguages, isLoading, loadingMessage, ocrRawResult, fullTextAnnotation, originalFullText, imageDimensions, detectedLanguageCode, detectedLanguageName, filterSettings, filterBounds, filteredSymbolsData, initialTextDirection, textDisplayMode, notification, isDimensionsKnown, maskedAreas, recognitionMode, tableSettings, currentTableComponent, useServerApiKey, serverApiAvailable,
    // Getters
