...
function autoDetectAndProcessTable(symbols) {
  ...
  console.log(`智能识别出${lines.length}行文本`)

  // 第2步：分析表格结构（检测列数和可能的分隔点）
  const tableStructure = analyzeTableStructure(lines)
  const { columnCount, columnSeparators } = tableStructure

  console.log(`检测到可能的列数: ${columnCount}`)
  console.log(`列分隔位置: ${columnSeparators.join(', ')}`)

  // 第3步：根据检测到的结构构建表格数据
  const tableRows = constructTableRows(lines, columnSeparators)
      ...
      .every(
        (row) =>
          !row.every(
            (cell, index) =>
              cell.length > 0 &&
              firstRow[index].length > 0 &&
              (cell.startsWith(firstRow[index]) || firstRow[index].startsWith(cell)),
          ),
      )

  const isLikelyHeader = !containsNumbers || isDistinctFromOtherRows

  if (isLikelyHeader && tableRows.length > 1) {
    // 使用第一行作为表头
    tableData.value = {
      headers: firstRow,
    ...
    // 生成默认表头
    const headers = new Array(columnsCount).fill('').map((_, index) => {
      return `列 ${index + 1}`
    })
...
function processTwoColumnTable(symbols) {
  ...
  textWithPositions.forEach((row) => {
    ...
    for (let index = 0; index < row.length - 1; index++) {
      const currentEnd = row[index].end
      const nextStart = row[index + 1].start
...
function calculateColumnPositions(symbols, rows, estimatedColumns) {
  ...
  rows.forEach((row) => {
    ...
    // 找出每行中的所有间隙
    for (let index = 0; index < sortedRow.length - 1; index++) {
      const current = sortedRow[index]
      const next = sortedRow[index + 1]
  ...
  rows.forEach((row) => {
    ...
    sortedRow.forEach((symbol) => {
      ...
      for (let colIndex = 0; colIndex < columnPositions.length; colIndex++) {
        if (symbolCenter < columnPositions[colIndex]) {
          columnIndex = colIndex
          break
        }
      }
...
// 按行分组的辅助函数，与groupSymbolsByLines功能相同
