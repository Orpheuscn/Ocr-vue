<template>
  <div>
    ...
    >
      <template #icon>
        <FloatingButtonIcons type="filter" />
      </template>
    </FloatingButton>
    ...
    >
      <div class="card bg-base-100 shadow-xl w-64 overflow-hidden rounded-xl">
        <div class="card-title p-2 justify-between items-center border-b border-base-300 text-sm">
          ...
          <button class="btn btn-xs btn-ghost btn-circle" @click="toggleVisibility">
            <svg
          ...
          <div class="flex flex-col gap-3">
            <div class="form-control">
              <label class="label py-0">
                <span class="label-text text-xs"
                  >{{ i18n.t('characterWidth') }}:
                  {{ formatRange(currentFilters.minWidth, currentFilters.maxWidth) }}</span
                >
              </label>
              <RangeSlider
            ...
            <div class="form-control">
              <label class="label py-0">
                <span class="label-text text-xs"
                  >{{ i18n.t('xCoordinate') }}:
                  {{ formatRange(currentFilters.minX, currentFilters.maxX) }}</span
                >
              </label>
              <RangeSlider
                :min="bounds.x.min"
            ...
            <div class="form-control">
              <label class="label py-0">
                <span class="label-text text-xs"
                  >{{ i18n.t('yCoordinate') }}:
                  {{ formatRange(currentFilters.minY, currentFilters.maxY) }}</span
                >
              </label>
              <RangeSlider
                :min="bounds.y.min"
...
const props = defineProps({
  bounds: {
    // Min/Max possible values for filters
    type: Object,
    required: true,
    default: () => ({
      width: { min: 0, max: 100 },
  ...
  initialFilters: {
    // Current filter values from store
    type: Object,
    required: true,
    default: () => ({
      minWidth: 0,
...
/* 原有的浮动按钮悬停效果已移动到 UI/FloatingButton.vue 组件中 */
