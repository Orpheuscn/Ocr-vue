      ...
      <div
        ...
        :class="{ 'with-grid': gridAlignmentToolRef?.showGridAlign }"
            :style="{ 
              ...
              backgroundSize: gridAlignmentToolRef?.showGridAlign ? `${gridAlignmentToolRef.gridCellSize}px ${gridAlignmentToolRef.gridCellSize}px` : 'auto'
            }"
      ...
      >
        ...
        <div
          ...
          :style="{
            ...
            border: gridAlignmentToolRef?.showGridAlign ? '1px solid rgba(200, 200, 200, 0.3)' : 'none',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
            fontWeight: gridAlignmentToolRef?.showGridAlign ? 'normal' : 'inherit',
            fontFamily: gridAlignmentToolRef?.showGridAlign ? '\'Arial\', sans-serif' : 'inherit',
...
const gridAlignmentToolRef = ref(null); // 添加对GridAlignmentTool的引用
...
onMounted(() => {
  ...
  // 在挂载完成后，确保gridAlignmentToolRef存在
  nextTick(() => {
...
// 监听过滤数据变化，更新字符显示
watch(() => store.filteredSymbolsData, (newData) => {
...
.coordinate-system.with-grid {
  ...
  background-position: 30px 0; /* Y轴的偏移 */
  /* backgroundSize会在style中设置 */
...
.text-block {
    ...
    cursor: default;
    user-select: none;
    pointer-events: none; /* 使鼠标事件穿透该元素 */
    padding: 0;
    box-sizing: border-box;
    /* 移除透明背景设置，允许通过style设置背景色 */
    border: 1px solid transparent;
    text-shadow: 0 0 2px var(--b2, white); /* 文字阴影增强可读性 */
}
...
/* 添加网格对齐时的文本块样式 */
.coordinate-system.with-grid .text-block {
  display: flex;
  align-items: center;
  justify-content: center;
  text-align: center;
  ...
  /* 移除背景颜色的设置，改为使用内联style提供的backgroundColor */
