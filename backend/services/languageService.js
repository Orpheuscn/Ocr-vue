...
// 获取语言名称
export function getLanguageName(code, preferredLang = "zh") {
  try {
    // 获取语言数据文件的路径
    ...
    if (!fs.existsSync(langFilePath)) {
      ...
      return code || (preferredLang === "zh" ? "未知" : "Undetermined");
    }
    ...
    return languageData[baseCode]
      ...
      : code || (preferredLang === "zh" ? "未知" : "Undetermined");
  } catch (error) {
...
// 获取所有可用语言的列表
export function getAllLanguages(preferredLang = "zh") {
  try {
    // 获取语言数据文件的路径
    const langFilePath = resolve(__dirname, "..", "data", "languages.json");
    ...
    return result.sort((a, b) =>
      a.name.localeCompare(b.name, preferredLang === "zh" ? "zh-CN" : "en")
    );
  } catch (error) {
