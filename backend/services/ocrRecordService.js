...
export const getUserOcrSummary = async (userId) => {
  try {
    ...
    const summary = {
      ...
      imageRecognitionMode: records.filter((r) => r.recognitionMode === "image_recognition").length,
...
// 获取用户使用过的语言统计
export const getUserLanguageStats = async (userId) => {
  try {
    // 确保userId是字符串类型
    const userIdStr = userId.toString();

    // 使用Mongoose聚合查询统计语言使用情况
    const languageStats = await OcrRecord.aggregate([
      { $match: { userId: userIdStr, status: "success" } },
      {
        $group: {
          _id: "$language",
          count: { $sum: 1 },
    ...
    // 过滤掉 "auto" 和空值，只返回有效的语言代码
    const validLanguages = languageStats.filter(
      (stat) => stat._id && stat._id !== "auto" && stat._id.trim() !== ""
    );
