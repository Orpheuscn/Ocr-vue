...
check_python_health() {
  ...
  while [ $retry -lt $max_retries ]; do
    # 请求Python服务健康检查端点
    local response=$(curl -s -m 5 http://localhost:5000/health)
    local curl_status=$?
    ...
    if [ $curl_status -eq 0 ] && [ ! -z "$response" ]; then
      ...
      if [ "$status" == "healthy" ]; then
        ...
        log "${YELLOW}Python服务健康检查部分成功: 状态为 $status (尝试 $((retry+1))/$max_retries)${NC}"
      fi
    else
      log "${YELLOW}Python服务健康检查失败: 端点不可用 (尝试 $((retry+1))/$max_retries)${NC}"
    fi
...
start_python_service() {
  ...
  # 使用start.sh脚本启动Python服务
  ./start.sh > "$APP_DIR/logs/python-service-$(date '+%Y%m%d-%H%M%S').log" 2>&1 &

  # 保存启动脚本的PID
  PYTHON_START_PID=$!
  echo $PYTHON_START_PID > "$APP_DIR/logs/python-service.pid"

  # 等待服务启动
  log "${BLUE}等待Python服务启动...${NC}"
  sleep 10
...
main() {
  ...
  # 检查Python服务进程 (现在使用Gunicorn)
  if check_process "gunicorn.*main:create_app" || check_process "python.*main.py"; then
    ...
    if check_python_health; then
      ...
      # 获取进程ID并终止
      PYTHON_PID=$(pgrep -f "gunicorn.*main:create_app" || pgrep -f "python.*main.py")
      if [ ! -z "$PYTHON_PID" ]; then
        log "${YELLOW}正在终止异常的Python服务进程 (PID: $PYTHON_PID)...${NC}"

        # 尝试优雅停止Gunicorn
        if pgrep -f "gunicorn.*master" > /dev/null; then
          log "${BLUE}尝试优雅停止Gunicorn...${NC}"
        ...
        # 检查是否仍有进程在运行
        REMAINING_PIDS=$(pgrep -f "gunicorn.*main:create_app" || pgrep -f "python.*main.py")
        if [ ! -z "$REMAINING_PIDS" ]; then
          log "${YELLOW}强制终止所有Python服务进程...${NC}"
