...
check_status() {
  ...
  # 检查Python服务
  PYTHON_PID=$(pgrep -f "python.*main.py" || pgrep -f "python.*server.py" || pgrep -f "gunicorn.*main:create_app")
  if [ ! -z "$PYTHON_PID" ]; then
    if pgrep -f "gunicorn.*main:create_app" > /dev/null; then
      echo -e "${GREEN}✓ Python服务(Gunicorn)正在运行 (PID: $PYTHON_PID)${NC}"
    else
    ...
    if curl -s http://localhost:$PYTHON_PORT/health > /dev/null; then
      echo -e "${GREEN}  - Python服务健康检查通过${NC}"
    else
      echo -e "${YELLOW}  - 警告: Python服务健康检查失败${NC}"
    fi
  else
    # 检查是否有Flask或Gunicorn进程在运行
    FLASK_PID=$(pgrep -f "flask run" || pgrep -f "gunicorn")
    if [ ! -z "$FLASK_PID" ]; then
      echo -e "${GREEN}✓ Python服务(Flask/Gunicorn)正在运行 (PID: $FLASK_PID)${NC}"
...
start_python_service() {
  ...
  # 检查Python服务是否已经运行
  if pgrep -f "python.*main.py" > /dev/null || pgrep -f "gunicorn.*main:create_app" > /dev/null; then
...
stop_python_service() {
  ...
  elif pgrep -f "python.*main.py" > /dev/null || pgrep -f "python.*server.py" > /dev/null; then
    echo -e "${YELLOW}未找到Python服务PID文件，但检测到Python服务进程${NC}"
    PYTHON_PID=$(pgrep -f "python.*main.py" || pgrep -f "python.*server.py")
  ...
  # 检查是否有Flask或Gunicorn相关进程仍在运行
  FLASK_PIDS=$(pgrep -f "flask run" || pgrep -f "gunicorn")
  if [ ! -z "$FLASK_PIDS" ]; then
    echo -e "${YELLOW}检测到Flask/Gunicorn进程仍在运行，尝试终止...${NC}"

    # 先尝试优雅停止Gunicorn
    if pgrep -f "gunicorn" > /dev/null; then
      echo -e "${BLUE}尝试优雅停止Gunicorn...${NC}"
      # 发送SIGTERM信号给Gunicorn主进程
      GUNICORN_MASTER=$(pgrep -f "gunicorn.*master" || pgrep -f "gunicorn.*main:create_app")
      if [ ! -z "$GUNICORN_MASTER" ]; then
        kill -SIGTERM $GUNICORN_MASTER
        sleep 3
      fi
    fi

    # 检查是否仍有进程在运行
    REMAINING_PIDS=$(pgrep -f "flask run" || pgrep -f "gunicorn")
    if [ ! -z "$REMAINING_PIDS" ]; then
      echo -e "${YELLOW}优雅停止失败，强制终止所有Flask/Gunicorn进程...${NC}"
      echo $REMAINING_PIDS | xargs kill -9
      sleep 1
    fi

    echo -e "${GREEN}Flask/Gunicorn进程已终止${NC}"
  fi
